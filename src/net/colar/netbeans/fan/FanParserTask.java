...
public class FanParserTask extends ParserResult
{
  ...
  CancellableRecoveringParserRunner runner;
  ...
  public void parse()
  {
    ...
    try
    {
      try
      {
        runner = new CancellableRecoveringParserRunner(parser.compilationUnit(), getSnapshot().getText().toString());
        parsingTask = runner.start();
        parsingResult = parsingTask.get();
  ...
  {
    System.out.print("Cleaning up failed / 1cancelled parser task.");
    if (parsingTask != null)
    {
  ...
  private FanResolvedType doCall(AstNode node, FanResolvedType type, AstNode forcedClosure)
  {
    ...
    if (type == null || type.getQualifiedType().equals(FanResolvedType.resolveThisType(node).getQualifiedType()))
    {
      ...
      if (childVar == null || (childVar.getKind() != VarKind.FIELD && childVar.getKind() != VarKind.METHOD))
  ...
  /**
   *
   * @param closureDefNode, can be null if not a closure defintion (itBlock / ctorblock)
   * @param blockNode
   * @param paramTypes
   * @return
   */
  ...
  private FanResolvedFuncType doClosureDef(AstNode closureDefNode, List<FanResolvedType> paramTypes)
  {
    ...
    if (closureDefNode != null)
    {	// fully qualified closure (with parameters)
  ...
  private GenericPair<String, FanResolvedType> doFormal(AstNode node, List<FanResolvedType> inferredTypes, int index)
  {
    ...
    return new GenericPair(formalName == null ? null : formalName.getNodeText(true), fType);
  }
