...
public class FanKeyStrokeHandler implements KeystrokeHandler
{
	...
	Pattern.compile("^\\s*case\\s*[^:]*:\\s*"),
	Pattern.compile("^\\s*default\\s*:\\s*"),
    //TODO: if closing bracket before -> no dedent.
    ...
    {
	//for some special patterns, decrease indent of current line (ex - deafult:)
	BaseDocument doc = (BaseDocument) document;
	...
	int indent = GsfUtilities.getLineIndent(doc, caretOffset);
        ...
        // TODO if user enters { after one of the identing patterns, we need to dedent
        /*if(line.startsWith("{") && DEINDENT_PATTERNS[i].matcher(prevLine).matches())
        {
        }*/
...
// within this ident level ?
    ...
    {
	for (int i = 0; i !=
		INDENT_AFTER_PATTERNS.length; i++)
	{
